...
if __name__ == '__main__':
    ...
    lm_outputs = model(input_ids=seqs)
    ...
    pos_emd_grad = next(model.named_parameters())[1].grad.clone()

    next(model.named_parameters())[1].grad = next(model.named_parameters())[1].grad * 0
    ...
    err1 = logits1.mean()
    err1.backward()
    pos_emd_grad1 = next(model.named_parameters())[1].grad



    torch.set_printoptions(precision=7)
    print("logits max", logits.abs().max())
    print("logits mean", logits.abs().mean())
    print("logits max diff:", (logits-logits1).abs().max())
    print("logits max mean:", (logits-logits1).abs().mean())
    print("pos_emd_grad max diff:", (pos_emd_grad1 - pos_emd_grad).abs().max())
    print("pos_emd_grad mean diff:", (pos_emd_grad1 - pos_emd_grad).abs().mean())
    ...
    #     lm_outputs = model(input_ids=seqs)
    #     logits_1 = lm_outputs['logits'] 
    #     err_1 = (logits_1 - 1).mean()
    #     err_1.backward()
    ...
    # for i in range(10): #warmup
    #     lm_outputs = model(input_ids=seqs)
    #     logits = lm_outputs['logits'] # [b, t, n_token]
    #     err = (logits - 1).mean()
    #     err.backward()
    #     print(logits.shape, err)
    # for i in tqdm(range(10)):
    #     lm_outputs = model(input_ids=seqs)
    #     logits = lm_outputs['logits'] # [b, t, n_token]
    #     err = (logits - 1).mean()
